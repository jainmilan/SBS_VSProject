...
public:
  ...
  */
  int indexarity() const { return internal::AMPL_EntityBase_indexarity(impl_); }
  ...
  */
  bool isScalar() const {
    return (internal::AMPL_EntityBase_indexarity(impl_) == 0);
  }
  /**
...
class BasicEntity : protected Entity {
public:
  ...
  */
  iterator find(TupleRef t) const { 
    return iterator(internal::EntityWrapper<InstanceClass>::find(impl(), t.impl())); }
  ...
  * @throws runtime_error if the entity has been deleted in the underlying %AMPL interpreter
  * @throws logic_error if the entity is scalar
  */
...
class EntityArgs {
  ...
  */
  const internal::EntityBase*const* getArgs() const {
    return &entities_[0];
  }
  /**
...
public:
  ...
  */
  ...
  The number of values in the array must be greater or equal to the
  specified size.
  
  \param	values	Values to be assigned.
  \param n Number of values to be assigned.
...
* Set::operator[]() or via Set::instances(),  which returns an InstanceList 
* providing iterators.
* <p>
...
public:
  ...
  */
  typedef SetInstance::MemberList MemberList;
  /**
  \rst
  ...
  */
  class InstanceList {
    friend class Set;
    const Set *parent_;
    explicit InstanceList(const Set *parent) : parent_(parent) {}
  ...
  /**
  * Get the InstanceList used to iterate over all the instances in
  * a %Set
  */
  InstanceList instances() const { return InstanceList(this); }
  ...
  */
  MemberList members() const { return get().members(); }
  /**
...
inline void Entity::setValues(DataFrame data) {
  internal::AMPL_EntityBase_setValues(impl_, data.impl(), internal::ErrorInfo());
}
