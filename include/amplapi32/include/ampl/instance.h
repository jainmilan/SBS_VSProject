...
namespace ampl {
class Entity;
template <class InnerInstance> class BasicInstance;
template <class InstanceClass> class BasicEntity;
...
*/
class Instance {
  template <class InstanceClass> friend class BasicEntity;
...
public:
  ...
  */
  template <class InnerInstance> Instance(BasicInstance<InnerInstance> instance);
  /**
  Operator for base class conversions
  */
  template <class InnerInstance> Instance &operator=(BasicInstance<InnerInstance> instance);
...
*/
template <class InnerInstance> class BasicInstance : private Instance {
...
public:
  ...
  */
  ...
  public:
    ...
    Constructor
    */
    explicit MemberList(internal::SetInstance *impl_) : size_(0) {
      members_ =
    ...
    */
    class iterator : public std::iterator<std::forward_iterator_tag, TupleRef> {
      friend class SetInstance::MemberList;
    ...
    public:
      ...
      */
      value_type operator*() const { return TupleRef(*ptr_); }
      /**
      Postfix increment
      */
      iterator &operator++() {
  ...
  */
  MemberList members() const { return MemberList(impl()); }
