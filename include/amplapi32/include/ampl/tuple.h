...
*/
template <bool OWNING> class BasicTuple {
  friend class internal::TupleBuilder;
  friend class Tuple;
  template <bool O> friend class BasicTuple; // For coercing
...
public:
  ...
  */
  template <bool U> BasicTuple(BasicTuple<U> const &other) {
    assign(other.impl_);
  }
  /**
  Destructor
  ...
  */
  template <bool U> BasicTuple &operator=(BasicTuple<U> other) {
    using std::swap;
    swap(*this, other);
    if (OWNING && (!U))
      other.destroy();
